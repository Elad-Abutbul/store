  ...
  return (
    <BrowserRouter>
      ...
      <Routes>
        ...
        <Route path={ROUTES.SEARCH} element={<Search url={URL.ADDTOCART} />} />
        <Route path={ROUTES.CART} element={<Cart />} />

        <Route path={ROUTES.PROFILE} element={<Profile />}>
          <Route path={ROUTES.EDIT} element={<SignUp/>} />
          <Route path={ROUTES.DELETEACCOUNT} element={<DeleteAccount />} />
          <Route path={ROUTES.VIEWPURCHASES} element={<ViewPurchases />} />
        </Route>
        <Route path={ROUTES.PAGENOTFOUND} element={<h2>page not found..</h2>} />
      </Routes>
    </BrowserRouter>
  );
}
